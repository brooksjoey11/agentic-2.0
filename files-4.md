...
1. **Single-threaded** - One conversation, one agent, blocking calls
2. **Stateless context** - Loses state between sessions
3. **Local-only** - Can't distribute workload
4. **No persistence layer** - Flat files only
5. **No specialized agents** - One agent does everything
6. **No streaming** - Blocking calls only
7. **No tool registry** - Hardcoded tools
8. **No multi-modal** - Text only
